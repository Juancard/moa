...
public class MultilabelFromOriginalDataset extends AbstractOptionHandler implements InstanceStream {
    ...
    private HashSet generateSet() {
        ...
        while(!has_labels){  // to ensure that the generated sample has at least one label
            for (int j : indices) {
                //y[j] ~ p(j==1|y)
                y[j] = (joint(j, y) > m_MetaRandom.nextDouble()) ? 1 : 0;
            }
            has_labels = IntStream.of(y).anyMatch(x -> x == 1); // checks if there is at least one sample with a label.
        }
        return vector2set(y);
    }
